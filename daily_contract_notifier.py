...
ADMIN_ERROR_TEMPLATE = "admin_alert" # User needs to create this
SHEET_ID = '1dKKrxe3oqX9mpEKU9PZFO947ZaELWfMqQcFsU0MgdY'
EXPORT_URL = f'https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=xlsx'
ADMIN_PHONE_NUMBER = "789969129844" # Using the verified number mapping
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                print(f"Warning parsing date for {name} ({date_exp}): {e}. Sending anyway to be safe, or skip? Skipping to avoid spam.")
                # continue # Uncomment to skip on date error
                # For now, let's proceed if date is unparseable? Or better to skip/alert?
                # User requirement is strict on filtering. If we can't parse, we can't filter.
                # However, sending to everyone was the old behavior.
                # Let's skip and log error for now, clearer.
            ...
            if success:
                ...
                # Clean error JSON if possible, but keep it simple
        ...
        # Fatal error, try to notify admin too if possible?
        # send_error_notification([f"Fatal Script Error: {e}"])
        pass
...
def main():
    ...
    while True:
        ...
        # If 9:00 AM has already passed today, verify if we should wait for tomorrow
        if now >= target:
             # Check if it's "close enough" (e.g. within 1 minute) to run NOW?
             # User said "load at 09:00". If started at 09:05, maybe run immediately?
             # For strictness, I'll schedule for tomorrow if passed.
        ...
        # Sleep a bit to ensure we don't double trigger (though logic above handles it via > target)
        time.sleep(60)
